...
{
	//m_sVoxelData.push_back(string("MushHouse"));
	//m_sVoxelData.push_back(string("Wood"));
	m_sVoxelData.push_back(string("Test_1"));
	m_sVoxelData.push_back(string("Test_2"));
}
	...
	for (_int z = 0; z < Scale.x; ++z)
	{
		for (_int x = 0; x < Scale.z; ++x)
		{
			...
			if (!CCubeManager::Get_Instance()->PosCheck(vTempPos))
			{
				...
				else if (CImGui_Manager::Get_Instance()->Get_Mode() == CImGui_Manager::MODE_TOTAL)
				{
